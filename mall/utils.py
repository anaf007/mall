...
def send_email(body):
    msg = Message(subject='隔壁小超市错误提醒%s'%dt.datetime.now().strftime('%Y%m%d%H%M%S'),sender=current_app.config['MAIL_USERNAME'],recipients=[current_app.config['MAIL_RECIPIENTS_NAME']])
    msg.html = body
    mail.send(msg)
